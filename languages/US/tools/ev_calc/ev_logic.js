...
window.onload = function() {
    fetch('../../../../data/ev_data_en.json')
        ...
        .catch(error => {
            console.error(error);
            document.getElementById('status-msg').innerText = "⚠️ Error: Open this with Live Server (CORS).";
...
function updateTotals() {
    ...
    if (currentTotal < MAX_TOTAL_EVS) {
        ...
        msg.innerText = `${MAX_TOTAL_EVS - currentTotal} EVs remaining.`;
        msg.style.color = "#aaa";
    } else if (currentTotal === MAX_TOTAL_EVS) {
        fill.style.backgroundColor = "#00e676"; 
        msg.innerText = "EVs Complete! (510/510)";
        msg.style.color = "#00e676";
    } else {
        fill.style.backgroundColor = "#ff5252"; 
        msg.innerText = `⚠️ LIMIT EXCEEDED! ${currentTotal - MAX_TOTAL_EVS} extra EVs.`;
...
function calculateHordes() {
    ...
    if (Object.keys(evData).length === 0) {
        container.innerHTML = `<p style="color:red; text-align:center;">Error: Data not loaded (Use Live Server).</p>`;
        return;
    }

    if (updateTotals() !== MAX_TOTAL_EVS) {
        container.innerHTML = `<div style="background:rgba(255,82,82,0.1); border:1px solid #ff5252; padding:20px; text-align:center; border-radius:8px; color:#ff5252;"><strong>⚠️ Total must be exactly 510 EVs.</strong></div>`;
        return;
    }
...
function generateResultCard(statName, targetEVs, info, hordesCount, multiplier, container) {
    ...
    let itemText = "No Item / Exp. Share";
    if (multiplier === 2) itemText = "Macho Brace (x2)";
    ...
    card.innerHTML = `
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
            <strong style="font-size:1.2em; color:${colors[statName] || 'white'};">${statName} - Target: ${targetEVs}</strong>
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; font-size:0.95em; color:#ddd;">
            ...
            <div style="text-align:right;">
                🎯 <strong>Hordes:</strong> <span style="color:var(--highlight); font-size:1.4em; font-weight:bold;">${hordesCount}</span><br>
                <span style="font-size:0.8em; color:#888;">(Sweet Scent)</span>
            </div>
        </div>
    `;
