...
function createPokemonRecipe(stats, nature, isMainLine, requiredGender) {
    ...
    if (isMainLine) {
        ...
        stepTitle = `⚙️ PREPARING SACRIFICE ${stats.length}x${config.value}`;
        desc = `THIS STEP IS ONLY TO CREATE MATERIALS.\nYou need to create a strong Male to give to your Main Mother later.\n\n`;
        desc += `🔸 MOTHER (Sacrifice): Has [${commonStats.join(", ")}].\n`;
        desc += `🔹 FATHER (Sacrifice): Has [${statsForB.join(", ")}].`;
    }

    let childGenderCost = isMainLine ? "REQUIREMENT: FEMALE (Your Project)" : "REQUIREMENT: MALE (For later use)";
    ...
    steps.push({
        ...
        pA: { 
            ...
            gender: "Female", 
            role: isMainLine ? "MOTHER (Main)" : "MOTHER (Sacrifice)", 
        },
        pB: { 
            ...
            nat: false, 
            gender: "Male", 
            role: "FATHER (Sacrifice)", 
        },
        child: { 
            stats: stats, 
            nat: nature,
            note: isMainLine ? "ADVANCED PROJECT" : "SACRIFICE READY",
